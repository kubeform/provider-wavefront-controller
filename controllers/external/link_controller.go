...
// +kubebuilder:rbac:groups=external.wavefront.kubeform.com,resources=links,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=external.wavefront.kubeform.com,resources=links/status,verbs=get;update;patch

func (r *LinkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("link", req.NamespacedName)
...
func (r *LinkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &externalv1alpha1.Link{}); err != nil {
			klog.Error(err, "unable to set up auditor", externalv1alpha1.Link{}.APIVersion, externalv1alpha1.Link{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&externalv1alpha1.Link{}).
