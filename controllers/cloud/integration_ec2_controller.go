...
// IntegrationEc2Reconciler reconciles a IntegrationEc2 object
type IntegrationEc2Reconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationec2s,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationec2s/status,verbs=get;update;patch

func (r *IntegrationEc2Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationec2", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationEc2")
...
func (r *IntegrationEc2Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.IntegrationEc2{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.IntegrationEc2{}.APIVersion, cloudv1alpha1.IntegrationEc2{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.IntegrationEc2{}).
