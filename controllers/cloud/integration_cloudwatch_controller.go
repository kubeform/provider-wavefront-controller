...
// IntegrationCloudwatchReconciler reconciles a IntegrationCloudwatch object
type IntegrationCloudwatchReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationcloudwatches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationcloudwatches/status,verbs=get;update;patch

func (r *IntegrationCloudwatchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationcloudwatch", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationCloudwatch")
...
func (r *IntegrationCloudwatchReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.IntegrationCloudwatch{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.IntegrationCloudwatch{}.APIVersion, cloudv1alpha1.IntegrationCloudwatch{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.IntegrationCloudwatch{}).
