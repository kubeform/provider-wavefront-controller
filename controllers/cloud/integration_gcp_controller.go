...
// IntegrationGcpReconciler reconciles a IntegrationGcp object
type IntegrationGcpReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationgcps,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationgcps/status,verbs=get;update;patch

func (r *IntegrationGcpReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationgcp", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationGcp")
...
func (r *IntegrationGcpReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.IntegrationGcp{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.IntegrationGcp{}.APIVersion, cloudv1alpha1.IntegrationGcp{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.IntegrationGcp{}).
...
func (r *IntegrationGcpReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for IntegrationGcp")
			return nil
		}
		...
		resourceList := &cloudv1alpha1.IntegrationGcpList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list IntegrationGcp")
			return nil
		}
