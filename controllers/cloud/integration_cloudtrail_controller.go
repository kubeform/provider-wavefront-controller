...
// IntegrationCloudtrailReconciler reconciles a IntegrationCloudtrail object
type IntegrationCloudtrailReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationcloudtrails,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.wavefront.kubeform.com,resources=integrationcloudtrails/status,verbs=get;update;patch

func (r *IntegrationCloudtrailReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationcloudtrail", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationCloudtrail")
...
func (r *IntegrationCloudtrailReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.IntegrationCloudtrail{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.IntegrationCloudtrail{}.APIVersion, cloudv1alpha1.IntegrationCloudtrail{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.IntegrationCloudtrail{}).
