...
// JsonReconciler reconciles a Json object
type JsonReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dashboard.wavefront.kubeform.com,resources=jsons,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dashboard.wavefront.kubeform.com,resources=jsons/status,verbs=get;update;patch

func (r *JsonReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("json", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Json")
...
func (r *JsonReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dashboardv1alpha1.Json{}); err != nil {
			klog.Error(err, "unable to set up auditor", dashboardv1alpha1.Json{}.APIVersion, dashboardv1alpha1.Json{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dashboardv1alpha1.Json{}).
